    ...
    */
    ...
    private:
        ...
        {
             // Extraemos q1 y q2 del vector de estado
                double q1 = joint_positions_(0);
                double q2 = joint_positions_(1);

                // Calculamos las entradas de g(q)
                double g1 = (m1_ + m2_) *l1_* g_ * std::cos(q1) + (m2_) * g_ * l2_ * std::cos(q1 + q2);
                double g2 = (m2_) * g_ * l2_ * std::cos(q1 + q2);

                // Montamos el vector de torques que compensa la gravedad
