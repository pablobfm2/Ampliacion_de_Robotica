    ...
    */
    ...
    private:
        ...
        Eigen::VectorXd forward_kinematics()
        {
            ...
            Eigen::VectorXd x(2);
            float x1 = l1_*cos(joint_positions_[0])+l2_*cos(joint_positions_[0]+joint_positions_[1]);
            float x2 = l1_*sin(joint_positions_[0])+l2_*sin(joint_positions_[0]+joint_positions_[1]);
            x << x1, x2;
        ...
        {
            // Extraemos posiciones y velocidades
            ...
            // Precálculos de ángulos y sumas
            double q12    = q1 + q2;

            // 1) Jacobiano J(q)
            jacobian_(0,0) = -l1_*std::sin(q1) - l2_*std::sin(q12);
            jacobian_(0,1) = -          l2_*std::sin(q12);
            jacobian_(1,0) =  l1_*std::cos(q1) + l2_*std::cos(q12);
            jacobian_(1,1) =           l2_*std::cos(q12);

            // 2) Derivada temporal  J̇ = dJ/dt
            // derivadas:
            // d/dt[ -l1 sin(q1)      ] = -l1 cos(q1)*q1_dot
            // d/dt[ -l2 sin(q12)     ] = -l2 cos(q12)*q12_dot
            // d/dt[  l1 cos(q1)      ] = -l1 sin(q1)*q1_dot
            // d/dt[  l2 cos(q12)     ] = -l2 sin(q12)*q12_dot

            jacobian_derivative_(0,0) = -l1_*std::cos(q1)*q1_dot - l2_*std::cos(q12)*q1_dot;
            jacobian_derivative_(0,1) = -l2_*std::cos(q12)*q2_dot;
            jacobian_derivative_(1,0) = -l1_*std::sin(q1)*q1_dot - l2_*std::sin(q12)*q1_dot;
            jacobian_derivative_(1,1) = -l2_*std::sin(q12)*q2_dot;
